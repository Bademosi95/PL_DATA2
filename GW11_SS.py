...
def build_feature_vector(home_team, away_team):
    ...
    NOTE: The app does not currently take a fixture kick-off date as input.
    For any date-dependent feature (e.g., rest_days_diff for an upcoming fixture),
    we use a safe fallback (0.0) rather than guessing.
    """

    # Helper: latest stat for a team, optionally by venue (is_home=1/0)
    def _latest(team, col, default=0.0, is_home=None):
        try:
    ...
    # Strength metrics from stats file (season-long)
    ...
    # Rolling / EWMA metrics (latest available)
    home_roll = {
        "points": _latest(home_team, "rolling_points"),
        "xg_for": _latest(home_team, "rolling_xg_for"),
        "xg_against": _latest(home_team, "rolling_xg_against"),
        "GD": _latest(home_team, "rolling_GD"),
        "fin": _latest(home_team, "rolling_finishing_overperf"),
        "def": _latest(home_team, "rolling_def_overperf"),
        "win_rate": _latest(home_team, "rolling_win"),
        "xg_for_var": _latest(home_team, "rolling_xg_for_var"),
        "xg_against_var": _latest(home_team, "rolling_xg_against_var"),
    }

    away_roll = {
        "points": _latest(away_team, "rolling_points"),
        "xg_for": _latest(away_team, "rolling_xg_for"),
        "xg_against": _latest(away_team, "rolling_xg_against"),
        "GD": _latest(away_team, "rolling_GD"),
        "fin": _latest(away_team, "rolling_finishing_overperf"),
        "def": _latest(away_team, "rolling_def_overperf"),
        "win_rate": _latest(away_team, "rolling_win"),
        "xg_for_var": _latest(away_team, "rolling_xg_for_var"),
        "xg_against_var": _latest(away_team, "rolling_xg_against_var"),
    }

    # Venue-specific time-weighted strengths
    home_tw_att = _latest(home_team, "tw_att_strength", is_home=1)
    home_tw_def = _latest(home_team, "tw_def_weakness", is_home=1)
    away_tw_att = _latest(away_team, "tw_att_strength", is_home=0)
    away_tw_def = _latest(away_team, "tw_def_weakness", is_home=0)
    ...
    features = {
        ...
        "rolling_points_diff": home_roll["points"] - away_roll["points"],
        "rolling_xG_diff": home_roll["xg_for"] - away_roll["xg_for"],
        "rolling_xGA_diff": home_roll["xg_against"] - away_roll["xg_against"],
        "rolling_GD_diff": home_roll["GD"] - away_roll["GD"],
        "finishing_overperf_diff": home_roll["fin"] - away_roll["fin"],
        "def_overperf_diff": home_roll["def"] - away_roll["def"],

        # New: form proxy
        "win_rate_diff": home_roll["win_rate"] - away_roll["win_rate"],

        # New: variance (sum-style features, matching weekly_update.py)
        "xg_var_sum": home_roll["xg_for_var"] + _latest(away_team, "rolling_xg_against_var", is_home=0),
        "xga_var_sum": home_roll["xg_against_var"] + _latest(away_team, "rolling_xg_for_var", is_home=0),

        # New: time-weighted strengths
        "tw_strength_diff": home_tw_att - away_tw_att,
        "tw_defense_diff": away_tw_def - home_tw_def,

        # New: rest days (date-dependent) -> safe fallback
        "rest_days_diff": 0.0,
    }

    # Ensure we return exactly the trained column order; fill any missing with 0.0
